...
// ............................................................
    ...
    var functions = {
        ...
        "_for": function(args, ruleset) {
            var counter = [1], vars = ["index", "count"];
            args = toArray(args);
            
            for (var i = 0, j = 0; i < args.length; i++) {
                var v = args[i];
                (v.type === "Dimension") // jshint expr: true
                    ? counter.unshift(v.value)
                    : vars[j++] = v.value;
            }
            
            if (counter.length < 2) throw {
                type:    "Argument",
                message: "missing `end` parameter"
            };

            return evalIterations(this.context, ruleset,
                counter[1], counter[0], vars, function(i, n) {
                    // todo: decide if the second variable should be `count` or `end`, hmm...
                    return [new Dimension(i), new Dimension(n)];
            });
        },
        
        // ....................................................

        "_for-each": function(list, args, ruleset) {
            var vars = ["value", "index"];
            list = toArray(list);
            args = toArray(args);
            if (!ruleset.ruleset) {
                ruleset = args[0];
            ...
            return evalIterations(this.context, ruleset,
                1, list.length, vars, function(i) {
                    return [list[i - 1], new Dimension(i)];
            });
        },
...
// helpers:
    ...
    function evalIterations(context, ruleset, start, end, vars, valuesFunc) {
        var d = (end - start) < 0 ? -1 : 1,
            n = Math.floor((end - start) * d + 1),
            i = start, count = n, r = [];
        // console.log(ruleset.frames);
        ruleset = ruleset.ruleset.rules;
        while (n--) {
            var rules  = ruleset.slice(0),
                values = valuesFunc(i, count);
            i += d;
            for (var j = 0; j < values.length; j++)
                rules.unshift(new Rule('@' + vars[j], values[j]));
            r = r.concat
                ((new Ruleset(null, rules)).eval(context).rules); 
        }
        
        console.log(context.frames);          
        return new DetachedSet(new Ruleset(null, r));
    }
